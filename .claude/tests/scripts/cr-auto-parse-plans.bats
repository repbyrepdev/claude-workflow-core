...
@test "DRIFT LOCK covers the CONFIG consumers too — triage clause + bootstrap mirror (p1r1)" {
	# The plan-me triage belt clause in .coderabbit.base.yaml (and its two
	# byte-locked mirrors) is a FOURTH consumer of the producer marker
	# strings. bootstrap-heredoc-parity + compose-regen lock the three
	# config copies to EACH OTHER, but only this test ties them to the
	# PRODUCER: a heredoc wording edit in skills/cr-plan/run.sh must fail
	# here naming every consumer, not silently un-arm the triage clause.
	local producer="$PLUGIN/skills/cr-plan/run.sh"
	local m1 m3
	m1=$(grep -o 'auto-created from CodeRabbit plan' "$producer" | head -1)
	m3=$(grep -oE '^Auto-generated by ..?cr-plan parse' "$producer" | head -1 | sed 's/\\//g')
	[ -n "$m1" ] && [ -n "$m3" ] || {
		echo "producer markers moved — update guards + triage clause (.coderabbit.base.yaml, .coderabbit.yaml, bootstrap-repo.sh heredoc) + this test"
		return 1
	}
	for cfg in "$PLUGIN/.coderabbit.base.yaml" "$PLUGIN/.coderabbit.yaml"; do
		grep -qF "$m1" "$cfg" || {
			echo "triage clause in $cfg lost producer marker: $m1"
			return 1
		}
		grep -qF "$m3" "$cfg" || {
			echo "triage clause in $cfg lost producer marker: $m3"
			return 1
		}
	done
	grep -qF "$m1" "$PLUGIN/scripts/bootstrap-repo.sh" || {
