...
# ---- tier PIN (#2544): the cap must not chase its own tail ----------------
#
# Everything above resolves the tier from the CURRENT finding count. That made
# the cap grow as the rounds it bounds found things: cap 3 (cr=10, moderate)
# became cap 5 the moment a round returned 13, so "3/3 ENFORCED" turned into
# "4/5". Observed live on fix/v0.34.184/2548-cr-thread-reply. It only ever
# rose, cr_count being floored by ancestor rows.
#
# The pin is per BRANCH, so a fix commit cannot re-resolve it one commit at a
# time. These drive the REAL script against the fixture repo; BATS_RUN_TMPDIR
# is exported so the pin lands in the test tmpdir, never the operator's tree.

_scaler_pinned() { # extra env words in $1
	run bash -c "cd '$WORK' && BATS_TEST_NAME='${BATS_TEST_NAME:-t}' BATS_TEST_TMPDIR='$TEST_TMP' ${1:-} bash '$REPO_ROOT/hooks/phase1-scaler.sh' --explain --base main"
}
...
@test "scaler pin: PHASE1_MIN_ROUNDS still raises a pinned-low cap" {
	...
	_scaler_pinned
	[[ $output == *"ROUNDS=2"* ]] || return 1
	_scaler_pinned "PHASE1_MIN_ROUNDS=4"
	[ "$status" -eq 0 ]
	[[ $output == *"ROUNDS=4"* ]] || {
...
@test "scaler pin: PHASE1_ROUNDS override still wins outright" {
	...
	_scaler_pinned
	[[ $output == *"ROUNDS=3"* ]] || return 1
	_scaler_pinned "PHASE1_ROUNDS=9"
	[ "$status" -eq 0 ]
	[[ $output == *"ROUNDS=9"* ]] || {
...
@test "scaler pin: --repin re-resolves deliberately" {
	...
	[[ $output == *"ROUNDS=3"* ]] || return 1
	_log_cr 13
	run bash -c "cd '$WORK' && BATS_TEST_NAME='${BATS_TEST_NAME:-t}' BATS_TEST_TMPDIR='$TEST_TMP' bash '$REPO_ROOT/hooks/phase1-scaler.sh' --explain --base main --repin"
	[ "$status" -eq 0 ]
	[[ $output == *"ROUNDS=5"* ]] || {
...
@test "scaler pin: a CORRUPT pin re-resolves loudly, never aborts" {
	...
	[[ $output == *"ROUNDS=3"* ]] || return 1
	local pin
	pin=$(find "$TEST_TMP/phase1-scaler" -name '*.json' 2>/dev/null | head -1)
	[ -n "$pin" ] || {
