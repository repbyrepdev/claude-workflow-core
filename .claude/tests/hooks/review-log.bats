...
@test "phase2 with a non-numeric findings count fails loud (exit 2 + clear message)" {
	...
	# (mirroring the phase1 5-arg guard) BEFORE jq --argjson, so a non-numeric
	# count exits 2 with a clear message instead of a cryptic jq abort.
	_init_repo
...
@test 'phase2 rejects a negative findings count (the ^[0-9]+$ guard, #2263)' {
	...
	# parsed as a count/flag. Pins the 'non-negative' half the abc-case omits.
	_init_repo
	_log phase2 -1 clean
	[ "$status" -eq 2 ]
	[[ $output == *"must be a non-negative integer"* ]]
