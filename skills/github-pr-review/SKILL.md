...
Run on demand to surface PR state before triaging review comments:

```bash
gh pr view --json number,title,state,milestone
```
...
LAST_PUSH=$(git log -1 --format=%cI @{u} 2>/dev/null || git log -1 --format=%cI HEAD)
...
gh api --paginate repos/$REPO/pulls/$PR/reviews/$REVIEW_ID/comments --jq '
  .[] | select(.in_reply_to_id == null) |
  {id, path, user: .user.login, created_at, body: .body[0:200]}
' | jq -s '.'
```
...
## CodeRabbit rate limit economy (5/hr per repo, Pro plan)

All sources share the same bucket:

| Action | Cost | Notes |
|---|---|---|
| PR push auto-review | 1 review | CodeRabbit auto-reviews each push |
| `coderabbit:review` command (plugin) | 1 review | Runs `coderabbit review --agent` |
| `coderabbit review --agent` raw CLI | 1 review | Same as above |
| `coderabbit:autofix` skill | **FREE** | Reads posted comments via `gh api graphql`, never calls CodeRabbit API |
| Thread replies via `gh api` | **FREE** | GitHub API only |

**Per PR budget:** initial push (1) + fix push (1) = 2 reviews used, 3 headroom. Autofix + replies are free — use freely.

**For iteration-heavy loops** (fix → re-review → fix): use `pr-review-toolkit:review-pr` (multi-agent, free, unlimited) as the local iteration loop. Only push once final pass is clean.

**For multi-round iteration with hard state transitions** (deploy → verify → adjust → deploy again until green): `ralph-loop:ralph-loop` is the right tool. Frees CodeRabbit budget entirely by keeping the loop local.

## CodeRabbit unavailability

If CodeRabbit hasn't posted a review within **10 minutes** of the PR push (no review body, no inline comments, status check missing):

1. Check CodeRabbit status: https://status.coderabbit.ai
2. Check the PR statusCheckRollup — if CodeRabbit shows `PENDING` or absent, it's stuck or the service is down
3. **Do NOT self-merge** to unblock — that bypasses review
4. Leave a comment on the PR: `CodeRabbit review pending for 10+ minutes. Status: [up/down]. Proceeding requires user approval.`
5. Ask the user: "CodeRabbit is unresponsive. Proceed without AI review or wait?"

Never skip CodeRabbit silently. The review is advisory-not-blocking per branch protection, but the workflow rule is that every push gets reviewed.
...
- **After applying all fixes + push** → auto-run `gh pr checks $PR --watch --fail-fast` to wait for CR re-review. If clean, GATE: present to user for merge approval.
- **CR still has outstanding threads after fix push** → reply to resolved threads with templates (no new reviews triggered — free). For new concerns, address in another fix commit (burns one more credit — accept the cost).
- **User approves merge** → auto-invoke github-pr-merge skill.
