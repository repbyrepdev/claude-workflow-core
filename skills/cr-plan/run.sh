...
issue_json=$(gh issue view "$issue" --json number,labels,title,state,body 2>"$issue_err") || {
...
parse)
	...
	# (2026-08-25 board explosion, p1r1 code-reviewer): these two guards
	# must live HERE — the real choke point every entrypoint funnels
	# through (auto-parse-plans poll, ship-pr-cycle `epic parse` exec, and
	# documented direct `APPROVE=1 run.sh parse <N>`). The poll wrapper
	# carries copies as a cheap early skip, but a caller-side-only guard
	# leaves the other two doors open to the exact recursion this fixes.
	#
	# Never decompose a parse OUTPUT: epics/subs this skill created carry
	# the auto-created markers in their generated bodies (the heredocs
	# ~150 lines below in THIS file — wording drift breaks the drift test
	# in cr-auto-parse-plans.bats). Three levels of scaffolding-of-
	# scaffolding (~30 issues) came through unguarded entrypoints.
	if echo "$issue_json" | jq -e '.body | test("auto-created from CodeRabbit plan|Auto-generated by `?cr-plan parse")' >/dev/null 2>&1; then
		echo "cr-plan: ERROR: issue #$issue is cr-plan SCAFFOLDING (its body carries the auto-created markers) — parse outputs are never parse inputs (2026-08-25 recursion guard)" >&2
		exit 2
	fi
	# Never decompose a non-OPEN source: scaffolding for closed work is
	# work-tracking for work that is already done (#2578 → #2623-#2625).
	if ! echo "$issue_json" | jq -e '.state == "OPEN"' >/dev/null 2>&1; then
		echo "cr-plan: ERROR: issue #$issue is not OPEN — closed/merged sources are never decomposed (2026-08-25 state guard)" >&2
		exit 2
	fi
	...
	# shellcheck disable=SC2001  # per-LINE prefix on a multiline var — parameter expansion cannot do this cleanly
