...
## Required linkage (ALL PR types)

Every template below must include these in the PR body — `pr-lint` blocks merge without them:

```markdown
Closes #<issue-number>
```

Plus: PR labels must match the linked issue's labels (area label REQUIRED: `monitoring`, `infrastructure`, `security`, or `performance`).
