...
No prose. No markdown fence. Just the array."
	...
	# Extract JSON array from output (Codex wraps response in agent envelope
	...
	if ! echo "$cleaned" | jq -e 'type == "array"' >/dev/null 2>&1; then
		# Codex emitted non-array (refusal, explanation). Count as 0.
		jq -nc --arg ts "$TS" --arg sha "$SHA" --arg agent "$agent" \
			'{ts:$ts, sha:$sha, phase:"0.5", cli:"codex", agent:$agent, findings:0, status:"non-array-output"}' \
			>>"$LOG"
		continue
	fi
	...
	jq -nc --arg ts "$TS" --arg sha "$SHA" --arg agent "$agent" --argjson n "$count" \
		'{ts:$ts, sha:$sha, phase:"0.5", cli:"codex", agent:$agent, findings:$n, status:"ok"}' \
		>>"$LOG"
