...
#
# The three SSOT files (1-3) are REQUIRED: an absent one is a precondition
# error (exit 2), never a silent skip — a label rename in a repo lacking its
# label SSOT means a wrong working dir / broken setup, and "done (nothing
# changed)" would hide it. Per present file: no-op when <old> is absent
# (idempotent — a partially applied migration completes cleanly on re-run);
# REFUSE (exit 2) on a target that is not parseable YAML; SKIP with a reconcile
# warning when <new> already exists, so a rename never clobbers an existing
# <new>. Step 4 (GitHub) skips when <old> is not a label or <new> already
# exists, and EXITS 2 if the GitHub API cannot be reached (never reported as
# success).
#
...
# (all of which carry the three SSOT files); run it from the repo root whose
# labels you are renaming.
#
# Usage:
...
#       a git tree; a required SSOT file absent; a target that is not valid
#       YAML; gh API unreachable)
...
# 1. labels.yml — rename the name field of the matching seq entry. The three
#    SSOT files are REQUIRED: an absent one is a precondition error (exit 2),
#    not a silent skip — a rename in a repo lacking its label SSOT signals a
#    wrong working dir / broken setup, and "done (nothing changed)" would hide it.
LABELS=".github/labels.yml"
[ -f "$LABELS" ] || {
	echo "migrate-label: required SSOT $LABELS is absent — refusing (run from the repo root whose labels you are renaming)" >&2
	exit 2
}
...
#    label->glob map; the renamed key is re-appended). Required SSOT (see above).
LABELER=".github/labeler.yml"
[ -f "$LABELER" ] || {
	echo "migrate-label: required SSOT $LABELER is absent — refusing (run from the repo root whose labels you are renaming)" >&2
	exit 2
}
...
#    Required SSOT (see above).
OVERLAY=".coderabbit.overlay.yaml"
[ -f "$OVERLAY" ] || {
	echo "migrate-label: required SSOT $OVERLAY is absent — refusing (run from the repo root whose labels you are renaming)" >&2
	exit 2
}
