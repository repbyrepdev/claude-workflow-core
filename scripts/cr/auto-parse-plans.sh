...
_check_and_parse_issue() {
	...
	if printf '%s' "$data" | jq -e '.body | test("auto-created from CodeRabbit plan|Auto-generated by `?cr-plan parse")' >/dev/null 2>&1; then
	...
	# OPEN skips.
	if ! printf '%s' "$data" | jq -e '.state == "OPEN"' >/dev/null 2>&1; then
