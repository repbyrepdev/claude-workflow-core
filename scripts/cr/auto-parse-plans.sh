...
# p1r1 simplifier: one predicate helper instead of six copies of the
# printf|jq -e plumbing — each guard below reads as just its condition.
# Reads $data from the caller's scope (bash dynamic scoping; only ever
# called from _check_and_parse_issue after $data is set).
_data_has() {
	printf '%s' "$data" | jq -e "$1" >/dev/null 2>&1
}

_check_and_parse_issue() {
	local issue=$1
	# Fetch issue labels + comments + createdAt (createdAt feeds the
	# plan-stuck timeout; label-applied waits are tracked via local
	# first-seen files below, NOT fetched events — p1r1 comment-analyzer)
	# + state/body for the 2026-08-25 guards.
	local data
	if ! data=$(gh issue view "$issue" --json labels,number,title,comments,createdAt,state,body 2>/dev/null); then
	...
	if _data_has '.labels[] | select(.name == "epic")'; then
		_log "skip-epic" "$issue" "skip" "epic label present — epics are parse outputs, not inputs"
		return 0
	fi

	# (2026-08-25 board explosion, the June runaway's sibling): the epic
	# guard above misses auto-created SUB-issues — ai-triage plan-me'd
	# them, CR planned them, and this parser decomposed decompositions
	# THREE levels deep (~30 issues, including scaffolding for a source
	# that closed mid-flight). ALL cr-plan outputs are parse-forbidden,
	# not just the epic-labeled half: skip any issue whose body carries
	# the auto-created markers. Body text is the durable signal — labels
	# can be lost, bodies of generated issues are never hand-rewritten.
	if _data_has '.body | test("auto-created from CodeRabbit plan|Auto-generated by `?cr-plan parse")'; then
		_log "skip-auto-scaffolding" "$issue" "skip" "body carries cr-plan auto-created markers — scaffolding is a parse output, never an input"
		return 0
	fi

	# State guard: the bulk poll lists --state open, but --issue <N> and
	# close-races (source closed after listing, e.g. by a batch PR merge)
	# reach here with a non-open issue. Decomposing a closed issue creates
	# work-scaffolding for work that is already done. Strict: anything but
	# OPEN skips.
	if ! _data_has '.state == "OPEN"'; then
		_log "skip-not-open" "$issue" "skip" "issue state is not OPEN — closed/merged sources are never decomposed"
		return 0
	fi

	# Require plan-me label (otherwise no plan to parse).
	if ! _data_has '.labels[] | select(.name == "plan-me")'; then
